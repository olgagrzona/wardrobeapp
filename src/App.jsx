...
const RATINGS=[{key:"love",icon:"👍",label:"Love it"},{key:"okay",icon:"😐",label:"It's okay"},{key:"never",icon:"👎",label:"Never again"}];
...
export default function App(){
  ...
  const unworn60=items.filter(i=>daysSince(i.lastWorn)>=60||(!i.lastWorn&&items.length>0));
  const mostWorn=[...items].sort((a,b)=>(b.wornCount||0)-(a.wornCount||0)).slice(0,3).filter(i=>i.wornCount>0);
    ...
    <div style={{fontFamily:"'Inter',sans-serif",minHeight:"100vh",background:T.bg,color:T.text}}>
      ...
      <div style={{maxWidth:1100,margin:"0 auto",padding:"18px 14px"}}>
        ...
        {view==="home"&&<div style={{maxWidth:680,margin:"0 auto"}}>
          ...
          <div style={{display:"grid",gridTemplateColumns:"repeat(3,1fr)",gap:10,marginBottom:28}}>
            {[{icon:"👗",label:"Items",value:items.length,action:"grid"},{icon:"✨",label:"Wishlist",value:wishlist.length,action:"wishlist"},{icon:"🎨",label:"Outfits",value:outfits.length,action:"outfits"}].map(({icon,label,value,action})=>(
              <div key={label} onClick={()=>setView(action)} style={{background:T.surface,borderRadius:12,padding:"16px 14px",border:`1px solid ${T.border}`,cursor:"pointer",textAlign:"center"}}>
            ...
            <div style={{display:"flex",flexDirection:"column",gap:10,marginBottom:24}}>
              ...
              <div style={{background:T.surface,borderRadius:12,padding:16,border:`1px solid ${T.border}`}}>
                ...
                <div style={{height:6,borderRadius:3,background:T.badge,overflow:"hidden"}}>
                  <div style={{height:"100%",borderRadius:3,background:wornThisMonth/items.length>0.5?"#2d6a4f":"#e67e22",width:`${Math.round(wornThisMonth/items.length*100)||0}%`,transition:"width 0.5s"}}/>
                </div>
                <div style={{fontSize:12,color:T.muted,marginTop:6}}>{wornThisMonth} of {items.length} items worn · {items.length-wornThisMonth} unworn this month</div>
              </div>
              ...
              {unworn60.length>0&&<div style={{background:T.surface,borderRadius:12,padding:16,border:`1px solid ${T.border}`}}>
                ...
                <div style={{fontSize:12,color:T.muted,marginBottom:10}}>Consider donating or selling pieces you never reach for</div>
                <div style={{display:"flex",gap:6,flexWrap:"wrap"}}>
                  {unworn60.slice(0,5).map(item=>(
                  ...
                  {unworn60.length>5&&<div style={{width:48,height:48,borderRadius:7,background:T.badge,display:"flex",alignItems:"center",justifyContent:"center",fontSize:11,color:T.sub}}>+{unworn60.length-5}</div>}
                </div>
              </div>}

              {/* Most worn */}
              {mostWorn.length>0&&<div style={{background:T.surface,borderRadius:12,padding:16,border:`1px solid ${T.border}`}}>
                <div style={{fontSize:13,fontWeight:600,color:T.text,marginBottom:10}}>⭐ Most worn</div>
                {mostWorn.map(item=>(
                  <div key={item.id} style={{display:"flex",alignItems:"center",gap:10,marginBottom:8}}>
                    <Img src={item.imageUrl} alt={item.name} style={{width:38,height:38,objectFit:"cover",borderRadius:6,flexShrink:0,display:"block"}}/>
                    <div style={{flex:1,minWidth:0}}>
                      ...
                      <div style={{fontSize:11,color:T.muted}}>{wornLabel(item).text}</div>
                    </div>
                    <div style={{fontSize:12,fontWeight:700,color:"#2d6a4f",flexShrink:0}}>{item.wornCount}x</div>
                  </div>
                ))}
              </div>}

              {/* Never worn */}
              {neverWorn.length>0&&<div style={{background:T.surface,borderRadius:12,padding:16,border:`1px solid ${T.border}`}}>
                <div style={{fontSize:13,fontWeight:600,color:T.text,marginBottom:4}}>🏷️ Never worn — {neverWorn.length} items</div>
                <div style={{fontSize:12,color:T.muted}}>{neverWorn.slice(0,3).map(i=>i.name).join(", ")}{neverWorn.length>3?` and ${neverWorn.length-3} more`:""}</div>
              </div>}
            </div>
          </>}
          ...
          <div style={{display:"flex",flexDirection:"column",gap:8}}>
            {[
              {icon:"➕",title:"Add Items",desc:"Paste a product URL for auto-fetch, or add manually with your own photo.",action:"add",color:"#f0faf4"},
              {icon:"👗",title:"Outfit Directory",desc:"Browse in an Instagram-style gallery. Drag to reorder, filter by brand, category or season.",action:"grid",color:"#f0f4ff"},
              {icon:"✨",title:"Wishlist",desc:"Save items you want. Track RRP vs sale price and move to wardrobe once bought.",action:"wishlist",color:"#fdf0ef"},
              {icon:"🎨",title:"AI Outfit Generator",desc:"Let AI build outfits from your wardrobe. Lock pieces you love and regenerate the rest.",action:"outfits",color:"#f5f0ff"},
            ].map(({icon,title,desc,action,color})=>(
              <div key={title} onClick={()=>setView(action)} style={{background:T.surface,borderRadius:12,padding:"14px 16px",border:`1px solid ${T.border}`,cursor:"pointer",display:"flex",gap:12,alignItems:"flex-start"}}>
                <div style={{width:38,height:38,borderRadius:9,background:color,display:"flex",alignItems:"center",justifyContent:"center",fontSize:18,flexShrink:0}}>{icon}</div>
        ...
        {view==="grid"&&<>
          ...
          {filtered.length===0?(
            <div style={{textAlign:"center",padding:"60px 20px",color:T.muted}}>
              ...
              <div style={{fontSize:16,fontWeight:700,color:T.sub,marginBottom:8}}>{items.length===0?"Your wardrobe is empty":"No items match these filters"}</div>
              {items.length===0&&<><div style={{fontSize:13,color:T.muted,marginBottom:16}}>Start building your digital wardrobe — add your first item below 👇</div><button style={pb} onClick={()=>setView("add")}>+ Add your first item</button></>}
            </div>
          ):(
            <div style={{display:"grid",gridTemplateColumns:"repeat(3,1fr)",gap:3}}>
              {filtered.map(item=>{
                const wl=wornLabel(item);
                const isDO=dragOver===item.id&&dragRef.current!==item.id;
                return(
                  <div key={item.id} draggable onDragStart={e=>onDragStart(e,item.id)} onDragOver={e=>onDragOver(e,item.id)} onDrop={e=>onDrop(e,item.id)} onDragLeave={()=>setDragOver(null)}
                    ...
                    <div style={{position:"absolute",inset:0,background:"linear-gradient(transparent 40%,rgba(0,0,0,0.8))",opacity:0,transition:"opacity 0.2s"}}
                    ...
                    {/* worn badge always visible bottom-left */}
                    ...
                    <div style={{position:"absolute",top:4,right:4,display:"flex",gap:3}}>
                      <button onClick={e=>{e.stopPropagation();markWorn(item.id);}} title="Mark worn today" style={{background:"rgba(0,0,0,0.55)",border:"none",borderRadius:4,padding:"3px 6px",cursor:"pointer",color:"#fff",fontSize:10}}>👗</button>
        ...
        {view==="wishlist"&&<>
          ...
          {wishlist.length===0?(
            <div style={{textAlign:"center",padding:"60px 20px",color:T.muted}}>
              ...
              <div style={{fontSize:13,marginBottom:16}}>Save items you're eyeing up — track prices and move them to your wardrobe when you buy 🛍️</div>
            ...
            <div style={{display:"grid",gridTemplateColumns:"repeat(auto-fill,minmax(185px,1fr))",gap:13}}>
              {wishlist.map(item=>{
                const rrp=pp(item.rrp),now=pp(item.priceNow),saving=rrp>0&&now>0&&rrp>now?rrp-now:0,ws=item.rrp?.match(/^[^0-9]+/)?.[0]||"£";
        ...
        {view==="outfits"&&<>
          ...
          {outfitTab==="generate"&&<>
            {items.length===0?(
              <div style={{textAlign:"center",padding:"60px 20px",color:T.muted}}>
                <div style={{fontSize:48,marginBottom:12}}>✨</div>
                <div style={{fontSize:16,fontWeight:700,color:T.sub,marginBottom:8}}>No items to work with yet</div>
                <div style={{fontSize:13,marginBottom:16}}>Add some clothes to your wardrobe first, then come back to generate outfits 👗</div>
                <button style={pb} onClick={()=>setView("add")}>+ Add your first item</button>
              </div>
            ):<>
              {genStep==="setup"&&<div style={{maxWidth:560}}>
                <div style={{fontSize:14,color:T.sub,marginBottom:14}}>Select categories to build your outfit from, then let AI pick the best combination.</div>
                <div style={{marginBottom:20}}>
                  ...
                  {cats.length===0?<div style={{color:T.muted,fontSize:13}}>No categories yet — create some first!</div>:(
                ...
                <button style={{...pb,opacity:genCats.length===0||genLoading?0.4:1,display:"flex",alignItems:"center",gap:8,justifyContent:"center",width:"100%"}} onClick={generateOutfit} disabled={genCats.length===0||genLoading}>
                  {genLoading?<><span style={{fontSize:18}}>⏳</span>Generating…</>:<><span style={{fontSize:18}}>✨</span>Generate Outfit</>}
                </button>
              </div>}
              {genStep==="result"&&<div style={{maxWidth:700}}>
                ...
                <div style={{fontSize:12,color:T.muted,marginBottom:10}}>🔒 Lock items to keep, then regenerate for new suggestions on the rest</div>
          ...
          {outfitTab==="create"&&<>
            {items.length===0?(
              <div style={{textAlign:"center",padding:"60px 20px",color:T.muted}}>
                ...
                <div style={{fontSize:13,marginBottom:16}}>Add items to your wardrobe first, then create your first outfit ✨</div>
                <button style={pb} onClick={()=>setView("add")}>+ Add your first item</button>
              </div>
          ...
          {outfitTab==="history"&&<>
            {outfits.length===0?(
              <div style={{textAlign:"center",padding:"60px 20px",color:T.muted}}>
                ...
                <div style={{fontSize:13,marginBottom:16}}>Create your first outfit — manually pick pieces or let AI do it ✨</div>
                  ...
                  return(<div key={outfit.id} style={{background:T.surface,borderRadius:12,border:`1px solid ${T.border}`,overflow:"hidden"}}>
                    <div style={{padding:"14px 16px",cursor:"pointer"}} onClick={()=>setExpandedOutfit(isOpen?null:outfit)}>
                      <div style={{display:"flex",justifyContent:"space-between",alignItems:"flex-start",marginBottom:10}}>
                        <div>
                          ...
                          <div style={{fontSize:11,color:T.muted,marginTop:2}}>{new Date(outfit.createdAt).toLocaleDateString()} · {outfit.itemIds.length} items{missing>0?` (${missing} removed)`:""}</div>
                          {outfit.notes&&<div style={{fontSize:12,color:T.sub,marginTop:3,fontStyle:"italic"}}>{outfit.notes}</div>}
                        </div>
                        <div style={{textAlign:"right"}}>
                          <div style={{fontSize:18,fontWeight:800,color:T.text}}>{S}{(outfit.totalPaid||0).toFixed(2)}</div>
                          {outfit.totalVal>outfit.totalPaid&&<div style={{fontSize:11,color:T.muted,textDecoration:"line-through"}}>{S}{(outfit.totalVal||0).toFixed(2)}</div>}
                        </div>
                      </div>
                      <div style={{display:"flex",gap:6,flexWrap:"wrap"}}>
                    ...
                    {isOpen&&<div style={{borderTop:`1px solid ${T.border}`,padding:"12px 16px"}}>
                      ...
                      <div style={{marginBottom:14}}>
                        <div style={{fontSize:12,fontWeight:600,color:T.sub,marginBottom:8}}>How do you feel about this outfit?</div>
                        <div style={{display:"flex",gap:8}}>
                          {RATINGS.map(r=>(
                            <button key={r.key} onClick={()=>rateOutfit(outfit.id,r.key)}
                              style={{flex:1,padding:"8px 6px",borderRadius:8,border:`2px solid ${outfit.rating===r.key?"#6c63ff":T.border}`,background:outfit.rating===r.key?"#6c63ff":T.surface,color:outfit.rating===r.key?"#fff":T.text,cursor:"pointer",fontSize:12,fontWeight:outfit.rating===r.key?700:400,display:"flex",alignItems:"center",justifyContent:"center",gap:4}}>
        ...
        {view==="add"&&<div style={form}>
          ...
          {cats.length===0&&<div style={{...err,background:"#fffbea",color:"#92651a",marginBottom:14}}>
            👉 No categories yet — <span style={{cursor:"pointer",textDecoration:"underline"}} onClick={()=>setView("manage")}>create one first →</span>
          </div>}
